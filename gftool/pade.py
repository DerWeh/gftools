...
def FilterHighVarianze(abs_num=None, rel_num=None):
    # TODO: check abs_num, rel_num
    assert abs_num is None or rel_num is None
    assert abs_num is not None or rel_num is not None
    def filter_high_varianze(pade_iter):
        """filter_high_varianze XXX.

        Parameters
        ----------
        pade_iter : Iterator of (..., N_z) complex np.ndarray
        ...
        """
        # FIXME: change to convention that last axis is z (flatten z to filter)
        pade = np.array(list(pade_iter))
        sum = np.sum(pade, axis=0)
        ...
        for nn in range(N_pades - abs_num_):
            diff = N_pades*pade - sum  # 50% of time
